# Reels — Skripte + Captions (DE)

Platzhalter: `[Monat]` = geplanter Einzug (z. B. „Dezember“).  
Sprache: Deutsch. Kurze Sätze — für CapCut-Untertitel und laut Lesen.  
Kein Budget, kein Midi-Job, keine Dividende im gesprochenen Text.

Erste Kommentarzeile (unter **jedem** Reel posten):

```
München · 2 Zimmer · allein · langfristig · ab [Monat] · DM
```

---

## 1. Hero (30–45 s)

**Skript (sprechen):**

```
Hi, ich bin Björn.
Ich komme Ende [Monat] zurück nach München.
Ich suche eine 2-Zimmer-Wohnung — allein, langfristig.
Kein WG-Zimmer, eine eigene Wohnung.
Wenn ihr jemanden kennt, der vermietet oder frei wird:
teilt das bitte, oder schreibt mir.
Danke.
```

**Caption — Netzwerk:**

```
Ich komme [Monat] zurück nach München und suche eine 2-Zimmer-Wohnung — allein, langfristig.

Wenn ihr jemanden kennt oder das teilen könnt: sehr gerne. DM offen.
```

**Caption — Gruppe:**

```
Suche ab [Monat]: 2 Zimmer München, allein, langfristig.
Bitte nur ernst gemeinte Hinweise / Angebote. DM.
```

---

## 2. Suchprofil (~20 s)

**Skript:**

```
Kurz das Profil:
2 Zimmer, allein, langfristig.
Nur in der Stadt München — kein Umland.
Ich bin ruhig und pfleglich.
Haustiere und Rauchen: [ehrlich einsetzen — z. B. „keine Haustiere, Nichtraucher“].
Details und Referenzen per DM.
```

**Caption — Netzwerk:**

```
Was ich suche: 2 Zimmer, allein, langfristig, nur Stadt München (kein Umland).
Mehr in der DM / Mappe auf Anfrage. Teilen hilft.
```

**Caption — Gruppe:**

```
2 Zimmer · allein · langfristig · ab [Monat]
Nur Stadt München — kein Umland. [Haustiere/Rauchen]. DM.
```

---

## 3. Guter Mieter (~20 s)

**Skript:**

```
Als Mieter:
Ich zahle pünktlich, halte die Wohnung in Ordnung, und bleibe gerne lange.
Referenzen und Unterlagen schicke ich auf Anfrage.
Kein Drama, kein Partykeller.
```

**Caption — Netzwerk:**

```
Für Vermieter und Freunde, die vermitteln: ich bin auf langfristig und ruhig ausgelegt.
Mappe / Referenzen per DM.
```

**Caption — Gruppe:**

```
Langfristig, allein, zuverlässig. Unterlagen auf Anfrage. DM.
```

---

## 4. So hilft ihr (~15 s)

**Skript:**

```
So könnt ihr helfen — in zehn Sekunden:
Teilt das Reel.
Schreibt mir, wenn ihr jemanden kennt.
Oder stellt mich kurz vor.
Das reicht schon.
```

**Caption — Netzwerk:**

```
Teilen, Intro, oder kurzer Tipp — alles hilft. Danke.
```

**Caption — Gruppe:**

```
Suche 2 Zimmer München ab [Monat], allein. Tipps / Angebote per DM. Bitte teilen.
```

---

## 5. Nicht ImmoScout (~20 s)

**Skript:**

```
Portale kenne ich.
Ich poste hier, weil in München oft das Netzwerk entscheidet.
Wenn ihr eine Wohnung kennt oder jemanden, der vermietet:
schreibt mir lieber persönlich.
Ich antworte schnell und sachlich.
```

**Caption — Netzwerk:**

```
Netzwerk vor Formularflut. Wenn ihr einen Hinweis habt: DM.
```

**Caption — Gruppe:**

```
2 Zimmer München, allein, ab [Monat]. Lieber DM als Massenbewerbung. Danke.
```

---

## Verify

- Alle fünf Skripte ohne €-Betrag und ohne Job-/Dividenden-Story.
- Netzwerk- und Gruppen-Caption für jedes Stück vorhanden.
- `[Monat]` vor dem Dreh ersetzt.
